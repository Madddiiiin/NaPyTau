...
class ParserUnitTest(unittest.TestCase):
	def test_createsAnArgumentParserInstanceAndConfiguresIt(self):
		...
		argparse_module_mock, argument_parser_mock, cli_arguments_module_mock = set_up_mocks()
		with patch.dict('sys.modules', {
			...
			self.assertEqual(
				...
				(('--filename',), {'type': str, 'help': 'File to ingest (only for mockup, not used in real application)'})
			)
	...
	def test_returnsACLIArgumentsInstanceFromTheParsedArguments(self):
		...
		with patch.dict('sys.modules', {
			...
			parse_cli_arguments()
			self.assertEqual(cli_arguments_module_mock.CLIArguments.mock_calls[0], ((test_args,),))
